  ...
  object implicits {
    ...
    implicit val polygonCollection: RenderSvg[Polygon] =
      new RenderSvg[Polygon] {
        def toSvg(base: Container, a: Polygon): Container = {
          ...
          polygon.attr("vector-effect", "non-scaling-stroke")
    ...
    implicit val renderJtsShapeList: RenderSvg[NonEmptyList[JtsShape]] =
      new RenderSvg[NonEmptyList[JtsShape]] {
        def toSvg(base: Container, a: NonEmptyList[JtsShape]): Container = {
          // val factory = a.head.g.factory
          val composite = a.map(_.g).reduce(svg.geometryUnionSemigroup)
          a.map(_.g).toList.map(_.toSvg(base))
          val envelope  = composite.getBoundary.getEnvelopeInternal
          base.viewbox(envelope.getMinX, envelope.getMinY, envelope.getWidth, envelope.getHeight)
          base.addClass("jts-list")
          base
        }
      }
    ...
    implicit val renderJtsShapeListWithIds: RenderSvg[NonEmptyList[(String, JtsShape)]] =
      new RenderSvg[NonEmptyList[(String, JtsShape)]] {
        def toSvg(base: Container, a: NonEmptyList[(String, JtsShape)]): Container = {
